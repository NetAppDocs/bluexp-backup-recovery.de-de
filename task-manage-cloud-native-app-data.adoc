...
Die REST-APIs zum Schutz der Applikationen in der Cloud sind verfügbar unter: https://[].

Sie sollten das Benutzer-Token mit gebündelter Authentifizierung erhalten, um auf DIE REST-APIs zuzugreifen. Informationen zum Abrufen des Benutzer-Tokens finden Sie unter https://["Erstellen Sie ein Benutzer-Token mit gebündelter Authentifizierung"].
