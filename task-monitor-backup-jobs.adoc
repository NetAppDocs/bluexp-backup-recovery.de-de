...
Auf der Registerkarte „Job Monitoring“ stehen sowohl Backup- und Restore-Vorgänge, die Sie über die BlueXP Backup- und Recovery-UI und -API initiiert haben, als auch systeminitiierte Jobs wie laufende geplante Backup-Vorgänge zur Verfügung. Dies gilt jedoch nur für Cloud Volumes ONTAP Systeme mit ONTAP 9.13.0 oder höher sowie für lokale ONTAP Systeme mit ONTAP 9.13.1 oder höher. Bei der Ausführung früherer ONTAP-Versionen werden nur vom Benutzer initiierte Jobs angezeigt.
...
. So zeigen Sie zusätzliche Spalten an (Arbeitsumgebung, SVM, Benutzername, Workload, Richtlinienname, Und Snapshot-Bezeichnung), wählen Sie aus image:button_plus_sign_round.png["Die Plus-Zeichen-Taste"].
...
NOTE: Job-Details werden für ONTAP-Backup-Jobs (Ad-hoc und geplant, geplante Jobs werden nur für ONTAP 9.13.0 oder höher angezeigt) sowie für alle Wiederherstellungsjobs angezeigt. Job-Details werden für laufende und abgeschlossene Jobs angezeigt.
...
Das Benachrichtigungscenter zeigt zahlreiche Sicherungs- und Wiederherstellungsereignisse an, aber nur die folgenden Ereignisse lösen E-Mail-Warnungen aus:
