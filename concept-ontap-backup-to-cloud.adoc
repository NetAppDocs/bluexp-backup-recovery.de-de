...
* WORM-/Compliance-Modus auf einem Objektspeicher wird derzeit nur von Amazon S3 und StorageGRID unterstützt. Dies wird als DataLock-Funktion bezeichnet und muss mithilfe von Cloud Backup-Einstellungen gemanagt werden, nicht über die Cloud-Provider-Schnittstelle.
...
* Die _hohe_ Wiederherstellungspriorität wird beim Wiederherstellen von Daten von Azure auf StorageGRID Systemen nicht unterstützt.
