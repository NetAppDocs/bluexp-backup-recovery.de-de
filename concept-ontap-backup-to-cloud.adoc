...
+
** In AWS, https://["Amazon Athena"^] Und https://["AWS Klue"^] Ressourcen werden in einem neuen S3-Bucket implementiert.
+
endif::aws[]
...
+
** In Azure, an https://["Azure Synapse Workspace"^] Und https://["Azure Data Lake Storage"^] Werden in Ihrem Storage-Konto bereitgestellt, um Ihre Daten zu speichern und zu analysieren.
+
endif::azure[]
...
* In Google wird ein neuer Bucket implementiert, und der https://["Google Cloud BigQuery Services"^] Werden auf Konto-/Projektebene bereitgestellt.
...
* In AWS ermöglicht BlueXP das https://["Amazon S3 Block – Public Access-Funktion"^] Auf dem S3-Bucket.
...
* In Azure verwendet BlueXP eine neue oder vorhandene Ressourcengruppe mit einem Storage-Konto für den Blob-Container. BlueXP https://["Blockiert den öffentlichen Zugriff auf Ihre BLOB-Daten"] Standardmäßig.
