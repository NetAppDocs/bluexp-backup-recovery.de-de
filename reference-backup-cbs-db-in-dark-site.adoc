...
Wenn Sie BlueXP Backup und Recovery in einer SaaS-Umgebung nutzen, bei der BlueXP Connector bei Ihrem Cloud-Provider oder auf Ihrem eigenen Host-System mit Internetzugang implementiert wird, werden alle wichtigen BlueXP Backup- und Recovery-Konfigurationsdaten gesichert und in der Cloud gespeichert. Wenn Sie BlueXP Backup- und Recovery-Funktionen an einer Site ohne Internetzugang nutzen, auch als „Dark Site“ bezeichnet, werden diese BlueXP Backup- und Recovery-Informationen ausschließlich auf dem lokalen Connector-System gespeichert.

In diesem Thema wird beschrieben, wie Sie kritische BlueXP Backup- und Recovery-Daten auf Ihrem vernetzten StorageGRID System sichern. Außerdem wird beschrieben, wie die Daten bei Bedarf auf einem neuen Konnektor wiederhergestellt werden.
