...
[role="lead"]
Wenn Sie die BlueXP backup and recovery an einem Standort ohne Internetzugang verwenden (der sogenannte _private Modus_), werden die Konfigurationsdaten der BlueXP backup and recovery im StorageGRID oder ONTAP S3-Bucket gesichert, in dem Ihre Sicherungen gespeichert werden. Wenn Sie ein Problem mit dem BlueXP Connector-Hostsystem haben, können Sie einen neuen Connector bereitstellen und die kritischen BlueXP backup and recovery und Wiederherstellungsdaten wiederherstellen.
