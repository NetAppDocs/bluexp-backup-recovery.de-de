...
Sie haben die Wahl, ob Sie Ihr lokales ONTAP-Cluster über das öffentliche Internet direkt mit Azure verbinden oder ob Sie ein VPN oder Azure ExpressRoute verwenden und den Datenverkehr über eine private VPC-Endpunktschnittstelle zu Azure weiterleiten.
...
Verifizieren Sie die Lizenzanforderungen sowohl für Azure als auch für BlueXP:
