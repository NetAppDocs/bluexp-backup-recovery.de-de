...
[role="lead"]
Die Verwaltung von Kubernetes-Workloads in BlueXP backup and recovery ermöglicht Ihnen die zentrale Erkennung, Verwaltung und Sicherung Ihrer Kubernetes-Cluster und -Anwendungen. Sie können die auf Ihren Kubernetes-Clustern gehosteten Ressourcen und Anwendungen verwalten. Außerdem können Sie Schutzrichtlinien erstellen und Ihren Kubernetes-Workloads zuordnen – alles über eine einzige Oberfläche.
...
BlueXP backup and recovery bietet die folgenden Vorteile für die Verwaltung von Kubernetes-Workloads:
