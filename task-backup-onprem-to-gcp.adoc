...
Sie haben die Wahl, ob Sie Ihren lokalen ONTAP-Cluster über das öffentliche Internet direkt mit Google Cloud Storage verbinden möchten oder ob Sie ein VPN oder Google Cloud Interconnect verwenden und den Datenverkehr über eine private Google Access-Schnittstelle leiten möchten, die eine private IP-Adresse verwendet.
