...
[role="quick-margin-para"]
Optional können Sie für die Datenverschlüsselung eigene benutzerdefinierte gemanagte Schlüssel einrichten, ohne die Standardschlüssel von Google Cloud zu verwenden. <<Prepare Google Cloud storage as your backup target,Erfahren Sie, wie Sie Ihre Google Cloud-Umgebung für ONTAP-Backups vorbereiten.>>
