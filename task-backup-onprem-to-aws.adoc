...
[NOTE]
====
Wenn Sie Backups in AWS GovCloud oder AWS China Regionen erstellen, müssen Sie den AWS Ressourcennamen „arn“ in den beiden folgenden Berechtigungssätzen unter allen _Resource_ Abschnitten ändern:
...
Wenn Sie Ihre eigenen Buckets erstellen, sollten Sie einen Bucket-Namen verwenden, der mit „netapp-Backup“ beginnt. Wenn Sie einen benutzerdefinierten Namen verwenden müssen, bearbeiten Sie die Richtlinien im Abschnitt S3-Berechtigungen einrichten, um eine Ressourcen-ARN zu verwenden, die mit Ihrem eigenen benutzerdefinierten Bucket-Präfix übereinstimmt.
...
[NOTE]
====
Wenn sich Ihre benutzerdefinierten Buckets in AWS GovCloud oder AWS China befinden, müssen Sie den AWS-Ressourcennamen „arn“ unter allen _Resource_-Abschnitten in den Berechtigungen ändern:

* Für AWS GovCloud Regionen müssen Sie beispielsweise „AWS Regionen“ zu „aws-US-gov“ ändern `arn:aws-us-gov:s3:::*`.
* Für AWS China-Regionen müssen Sie „aws“ in „aws-cn“ ändern, z. B. `arn:aws-cn:s3:::-*`.
