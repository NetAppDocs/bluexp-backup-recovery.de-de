...
Sie können in der BlueXP  Backup- und Recovery-UI Snapshot-, Replizierungs- und Backup-to-Objekt-Storage-Richtlinien erstellen. Weitere Informationen finden Sie im Abschnittlink:task-manage-backups-ontap.html#add-a-new-backup-to-cloud-policy["Hinzufügen einer neuen Backup-Richtlinie"].
...
* Sie können Daten von lokalen ONTAP Systemen oder Cloud Volumes ONTAP Systemen in AWS Geschäftsregionen in Amazon S3 sichern. link:task-backup-to-s3.html["Backup von Cloud Volumes ONTAP Daten in Amazon S3"].
...
* Sie können Daten aus lokalen ONTAP Systemen oder Cloud Volumes ONTAP Systemen in Azure kommerzielle Regionen in Azure Blob sichern. link:task-backup-to-azure.html["Backup von Cloud Volumes ONTAP Daten in Azure Blob"].
