...
Anforderungen an Steckverbinder:: Der Connector kann in einer AWS-Region mit oder ohne Internetzugang installiert werden. Derzeit werden nur AWS Commercial Sites für Websites ohne Internetzugang unterstützt.
+
--
...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-install-connector-onprem-no-internet.html["Installation des Connectors auf einem Linux-Host in einer AWS-Region ohne Internetzugang"^]
