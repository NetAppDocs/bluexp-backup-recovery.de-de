...
. Kopieren Sie den Inhalt von *sudoer.txt* unter: _/var/lib/Docker/Volumes/Service-Manager[1]-2_cloudmanager_scs_Cloud_Volume/_Data/€(sudo docker ps_grep -Po „Cloud Manager_scs_Cloud:.*? „/sed -e s/ *€//' Cut -f2 -d“:“)/sc-linux-Host-Plugin_
...
.. Wählen Sie Plug-in-Host.
(Optional) Wenn der Host nicht hinzugefügt wird, klicken Sie auf *Add Plug-in Host*.
...
=== SAP HANA-Datenbankhost löschen

Sie können den SAP HANA-Datenbank-Host nur mit REST-APIs entfernen.

*Schritte*

. Löschen Sie alle Systeme, die mit dem SAP HANA-Datenbank-Host verbunden sind, über die UI oder über die unten stehende REST-API.
`DELETE /saphana/anf/systems/(id)`
. Führen Sie die folgenden Schritte aus, um den SAP HANA-Datenbank-Host zu entfernen.
+
.. Verwenden SIE DIE GET REST API, um die ID des zu löschenden SAP HANA Datenbank-Hosts zu erhalten.
`GET /saphana/hosts`
   `{`
    `"num_records": 1,`
    `"records": [`
...
`}`
    `]`
`}`
.. Löschen Sie den SAP HANA-Datenbank-Host, indem Sie die ID aus der unten stehenden REST-API übergeben.
`DELETE /saphana/hosts/(id)`
.. Deinstallieren Sie das SAP HANA Linux-Plug-in auf dem Datenbank-Host mit dem folgenden Befehl.
...
=== Ändern Sie den SAP HANA-Datenbankhost

Nach dem Hinzufügen des SAP HANA-Datenbankhosts können Sie den Hostnamen oder den Plug-in-Port mithilfe DER REST-API ändern.

*Schritte*

. Verwenden SIE DIE GET REST API, um die ID des zu löschenden SAP HANA Datenbank-Hosts zu erhalten. `GET /saphana/hosts`
   `{`
    `"num_records": 1,`
    `"records": [`
        `{`
...
`}`
    `]`
`}`
. Ändern Sie den Hostnamen oder den Plug-in-Port mithilfe der folgenden PATCH-API.
