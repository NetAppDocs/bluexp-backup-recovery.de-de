...
[role="lead"]
BlueXP Backup und Recovery (Cloud Backup) bietet effiziente, sichere und kostengünstige Datensicherung für NetApp ONTAP Daten, persistente Kubernetes Volumes, Datenbanken und Virtual Machines – lokal und in der Cloud. Backups werden automatisch erstellt und in einem Objektspeicher in Ihrem Public oder Private Cloud-Konto gespeichert.

Der Service führt eine dauerhaft inkrementelle Replizierung auf Blockebene durch und erhält die gesamte Storage-Effizienz aufrecht, wodurch sich die Menge der replizierten und gespeicherten Daten erheblich verringert. Außerdem bezahlen Sie nur für das, was geschützt ist, und nutzen die kostengünstigsten verfügbaren Storage-Tiers, was Cloud Backup zu einer sehr kostengünstigen Lösung macht.
...
https://bluexp.netapp.com/cloud-backup["Weitere Informationen zu Cloud Backup"^].
...
* Mit Cloud-Backup für Applikationen können Sie die applikationskonsistenten Snapshots aus lokalen ONTAP Systemen sichern. link:concept-protect-app-data-to-cloud.html["Detaillierte Funktionen finden Sie hier"].
...
TIP: Wenn der BlueXP Connector in einer Regierungsregion in der Cloud oder an einem Standort ohne Internetzugang (einem Dark Site) implementiert wird, unterstützt Cloud Backup nur Backup- und Restore-Vorgänge aus ONTAP Systemen. Beim Einsatz dieser Implementierungsmethoden unterstützt Cloud Backup Backup keine Backup- und Restore-Vorgänge aus Kubernetes-Clustern, Applikationen oder Virtual Machines.
