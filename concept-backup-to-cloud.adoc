...
[role="lead"]
Der BlueXP backup and recovery und Wiederherstellungsdienst bietet effizienten, sicheren und kostengünstigen Datenschutz für ONTAP Volumes, Microsoft SQL Server-Instanzen und -Datenbanken, VMware-Workloads (Vorschau) und Kubernetes-Workloads (Vorschau).


NOTE: Dokumentation zum Schutz von VMware- und Kubernetes-Workloads wird als Technologievorschau bereitgestellt. Bei diesem Vorschauangebot behält sich NetApp das Recht vor, Angebotsdetails, Inhalte und Zeitplan vor der allgemeinen Verfügbarkeit zu ändern.
...
** Verwenden Sie BlueXP backup and recovery zusammen mit SnapCenter.
...
** Verwenden Sie BlueXP backup and recovery ohne SnapCenter.
...
+
** Schützen Sie Ihre VMware-VMs und Datenspeicher mit BlueXP backup and recovery.
...
** Verwenden Sie BlueXP backup and recovery ohne SnapCenter Plug-in for VMware vSphere.
...
** Stellen Sie VM-Daten mit BlueXP backup and recovery aus der Cloud zurück in das lokale vCenter wieder her.  Sie können die VM an genau demselben Speicherort wiederherstellen, von dem die Sicherung erstellt wurde, oder an einem anderen Speicherort.
** Verwenden Sie BlueXP backup and recovery zusammen mit dem SnapCenter Plug-in for VMware vSphere.
...
* *Workload*: Ein Workload in BlueXP backup and recovery kann ONTAP Volumes, Microsoft SQL Server-Instanzen und -Datenbanken, VMware-VMs und -Datenspeicher oder Kubernetes-Cluster und -Anwendungen umfassen.
