...
Der Service führt eine dauerhaft inkrementelle Replizierung auf Blockebene durch und erhält die gesamte Storage-Effizienz aufrecht, wodurch sich die Menge der replizierten und gespeicherten Daten erheblich verringert. Außerdem bezahlen Unternehmen nur für das, was geschützt ist, und nutzen die kostengünstigsten verfügbaren Storage-Tiers, was Cloud Backup zu einer sehr kostengünstigen Lösung macht.
...
* Erstellen Sie mithilfe von Cloud-Backup für Applikationen ein Backup der applikationskonsistenten Snapshots aus lokalen ONTAP Systemen. link:concept-protect-app-data-to-cloud.html["Detaillierte Funktionen finden Sie hier"].
...
TIP: Wenn der BlueXP Connector in einer Regierungsregion in der Cloud oder an einer Website ohne Internetzugang (eine dunkle Site) bereitgestellt wird, unterstützt Cloud Backup nur Backup- und Restore-Vorgänge von ONTAP Systemen. Beim Einsatz dieser Implementierungsmethoden unterstützt Cloud Backup keine Backup- und Restore-Vorgänge aus Kubernetes-Clustern, Applikationen oder Virtual Machines.
